class User < ApplicationRecord
  ...
  def self.GetFriends(userid)
    query = "select id,name,email from users as u inner join ( select friend_b_id from friendships where friendships.friend_a_id = #{userid}) as d on u.id = d.friend_b_id;"
